...
| 如果你想... | 去这里 |
|---|---|
| **部署、启停、配置、运维、排查** | 👉 [**`docs/fork/deployment.md`**](docs/fork/deployment.md) — 本仓库唯一需要反复看的文档 |
| 了解本 fork 相对 upstream 改了什么 | [`docs/fork/deployment.md` §0](docs/fork/deployment.md#0-本-fork-相对-upstream-的改动) |
| 查文档全索引（fork vs upstream 原生） | [`docs/README.md`](docs/README.md) |
| 看 upstream 合并策略 | [`docs/private/reviews/merge_main_into_upstream_main.md`](docs/private/reviews/merge_main_into_upstream_main.md) |
...
本 fork **只改工程/部署层**（submodule 布局、统一 launcher、构建脚本、PyCharm run configs、apisource 切换）—— 业务代码保持与 upstream 一致以便合入。
