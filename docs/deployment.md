...
# 状态
rtk uv run python scripts/launcher.py status
...
**探活规则**：
- `ready_port = 0` → 跳过探活，立即继续下一个组件
- 探活失败两种情况：① 进程已退出（立即报 ERR 并跳过） ② 到超时仍无法 connect（报 WARN）
- napcat 的默认 `ready_port = 8095` 对应 OneBot WS 端口（NapCat 实际 listen），QQ + DLL 注入约 5-15s
- adapter 的 `ready_port = 0`：adapter 是纯 outbound WS 客户端（连 NapCat 和 bot），自身不 listen
- bot 无探活
...
仓库在 [.run/](../.run/) 下内置了 10 个即用的 PyCharm Run Configuration（PyCharm / IntelliJ 系会自动发现该目录）。首次使用需要做一次解释器绑定：

### 步骤一：把 `.venv` 绑为项目解释器

1. **File → Settings → Project: MaiBot → Python Interpreter**
2. 齿轮图标 → **Add Interpreter → Add Local Interpreter → Select existing**
3. 选 `d:/Toy/MaiBot/.venv/Scripts/python.exe`（`uv sync` 创建的 venv）
4. 确认 `.run/` 里 run configs 的 `Use specified interpreter` 勾选到该 SDK

> 这相当于 `uv run python` 的等价物：uv 创建的 `.venv` 就是运行现场。不需要再套一层 `uv run`。
...
| `Build NapCat` | `scripts/build_napcat.py` | 构建 NapCat shell（增量） |
| `Build NapCat (Clean)` | `scripts/build_napcat.py --clean` | 清空 `runtime/napcat` 后重建（保留 config/ 等） |
| `Launcher: Start All` | `scripts/launcher.py start` | 启动三件套，开 3 个 cmd 窗口 |
| `Launcher: Start Bot Only` | `scripts/launcher.py start bot` | 只起 bot |
| `Launcher: Start Adapter Only` | `scripts/launcher.py start adapter` | 只起 adapter |
| `Launcher: Start NapCat Only` | `scripts/launcher.py start napcat` | 只起 NapCat |
| `Launcher: Stop All` | `scripts/launcher.py stop` | 按反向顺序停 |
| `Launcher: Restart All` | `scripts/launcher.py restart` | 停后重启 |
| `Launcher: Status` | `scripts/launcher.py status` | PID / 运行状态 |
| `Sync Upstream (Dry Run)` | `scripts/sync_upstream.py` | 预览上游 diff，不改动 |
| `Sync Upstream (Apply)` | `scripts/sync_upstream.py --apply` | merge + push 所有仓 |
...
当前 [.gitignore](../.gitignore) 已排除 `.run`（个人配置不入库策略）。如需团队共享，把 `.run` 从 gitignore 去掉即可。
...
| `.run/` | 否 | 当前 `.gitignore` 已排除；PyCharm 运行配置本地维护 |
...
- 设计原委：[unified_workspace_plan.md](unified_workspace_plan.md)
- 历史迁移手册（从旧三目录迁到当前结构）：[unified_workspace_migration.md](unified_workspace_migration.md)
