...
## 2026-09-17 upstream main 同步基线

合并 `refs/remotes/upstream/main` 的 `39a1346c8`，吸收稳定版 WebUI、插件 LLM 能力参数解析和时间检索修复。保留本地人设参数化、模型配置、回复策略及源码前端强制构建规则。依赖锁按合并后的声明重新解析，实例部署配置与运行数据不进入共享提交。
