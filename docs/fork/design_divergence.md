...
## 2026-09-23 upstream dev 同步基线

合并 `refs/remotes/upstream/dev` 的 `4117691c0`，接纳回复压缩合并时保留分隔符、统计查询列投影和图片记忆启动核对异步化修复。

- 图片资产核对遵循上游取消与资源释放顺序，避免阻塞主循环和插件握手。
- 保留 fork 的回复纠正引用、供应商路由和双实例认证；实例配置、人设和本地部署修改独立维护。

## 2026-09-20 upstream dev 同步基线

合并 `refs/remotes/upstream/dev` 的 `3a791cde8`，接纳记忆时间线批量查询、原生图片嵌入协议、模型分时计价、思考格式和可选复古回复模板。

- WebUI 阻塞接口采用上游同步执行方式，保留按端口隔离的登录 Cookie；插件版本列表也使用实例 Cookie。
- 回复生成器同时保留 fork 的供应商路由和上游复古模板；现有角色模板、模型参数与服务配置独立维护。
- 保留本地 Qwen 温度与模型池，新增分时计价字段；配置版本跟随上游 `8.14.47` / `1.17.11`。

## 2026-09-17 upstream dev 同步基线

合并 `refs/remotes/upstream/dev` 的 `582266b87`：引入图片记忆、长期记忆导入导出和管理界面、插件发布版本选择及上下文用量显示。

- 保留 fork 的回复供应商路由、Qwen/BaiLian 模型配置、强制回复策略与摘要 JSON 解析修复。
- 摘要使用上游的中英日外部模板，其中已包含增量提取、用户事实边界和图片证据校验约束。
- 配置模板版本更新到 `8.14.43`；已有模型与实例配置独立保留，图片嵌入模型默认不指定。
- 内置适配器和插件管理模块声明支持宿主 `1.3.0`；第三方插件仍需各自确认兼容范围。
- 合并验证修正嵌入模型确认按钮调用、补齐摘要元数据，并对齐记忆来源、时间线和旧插件清单迁移的回归预期。
