...
> 调研日期：2026-06-21
> 适用仓库：`DogTwoMey/MaiBot` 当前工作树，以及 `Mai-with-u/MaiBot`、`Mai-with-u/MaiBot-Napcat-Adapter`、`NapNeko/NapCatQQ` 上游
...
当前 fork 同时保留了两条 QQ 接入路径：

1. [`external/adapter`](../../external/adapter/) 中的独立进程版 Adapter。
2. [`plugins/maibot-team_napcat-adapter`](../../plugins/maibot-team_napcat-adapter/) 中的 `MessageGateway` 插件版 Adapter。

官方 Adapter 上游已经把 `main` 从独立进程布局切换为插件布局。直接更新 `external/adapter` 到官方 `main` 会发生目录形态切换，不能把它当成普通的小版本升级。
...
3. 2026-04-03，官方 NapCat Adapter 上传完整插件化实现，使用 MaiBot `MessageGateway` 接入：
   ...
   - [当前插件入口](../../plugins/maibot-team_napcat-adapter/plugin.py)

4. 插件版 Adapter 会从 NapCat 的 `self_id` 获取账号，向 Host 上报 `account_id`，并把 `self_id`、`connection_id` 写入入站路由元数据：
   - [运行状态上报](../../plugins/maibot-team_napcat-adapter/runtime_state.py)
   - [入站路由构造](../../plugins/maibot-team_napcat-adapter/runtime/router.py)
...
- 插件版 NapCat Adapter 的配置模型只有一个 `napcat_server`，一个插件实例只维护一个传输连接：[插件配置](../../plugins/maibot-team_napcat-adapter/config.py)。
...
- 独立进程版 Adapter 使用全局 `server_connection` 和全局发送器。多个 NapCat 客户端同时连接同一个 Adapter 时，后连接会覆盖当前连接引用：[连接处理](../../external/adapter/main.py)、[消息处理器](../../external/adapter/src/recv_handler/message_handler.py)。
