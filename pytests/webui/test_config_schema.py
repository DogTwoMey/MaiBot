...
def test_memory_query_config_fields_are_exposed():
    ...
    assert memory_schema.get("uiParent") == "a_memorix"
...
def test_advanced_fields_are_marked_in_webui_schema():
    ...
    assert "normal_field" in fields_by_name
    assert fields_by_name["normal_field"].get("advanced") is None
    assert fields_by_name["advanced_field"].get("advanced") is True
