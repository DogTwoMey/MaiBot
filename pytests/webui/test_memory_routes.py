...
def test_import_chat_targets_route(client: TestClient, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: _FakeDbContext(
            SimpleNamespace(exec=lambda statement: SimpleNamespace(
                all=lambda: [] if "GROUP BY" in str(statement) else [chat_session], first=lambda: None
            ))
        ),
    )
    ...
    class _FakeSession:
        ...
        def exec(self, statement):
            ...
            return _FakeExecResult([(
                message.session_id, message.group_id, message.group_name,
                message.user_id, message.user_cardname, message.user_nickname, 1.0,
            )])
