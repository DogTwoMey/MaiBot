...
def test_reply_necessity_ignores_bot_self_messages(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "src.maisaka.turn_scheduler.is_bot_self",
        lambda platform, user_id: platform == "qq" and user_id == "2814567326",
    )
    runtime = _runtime()
    score, _ = runtime._message_turn_scheduler.score_reply_necessity(
