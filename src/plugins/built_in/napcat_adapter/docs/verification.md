...
- 扫描 `src/plugins/built_in/napcat_adapter/apis/*.py` 中全部 `@API(..., public=True)` 公开接口。
...
- 对强类型封装 API，额外对照 `src/plugins/built_in/napcat_adapter/services/query_service.py` 确认适配器实际下发的 body。
