...
def get_emoji_detail(emoji_id: int, maibot_session: Optional[str] = auth_cookie()) -> EmojiDetailResponse:
    """获取表情包详细信息。
...
def delete_emoji(emoji_id: int, maibot_session: Optional[str] = auth_cookie()) -> EmojiDeleteResponse:
    """删除表情包。
...
def get_emoji_stats(maibot_session: Optional[str] = auth_cookie()) -> Dict[str, Any]:
    """获取表情包统计数据。
...
def ban_emoji(emoji_id: int, maibot_session: Optional[str] = auth_cookie()) -> EmojiUpdateResponse:
    """禁用表情包。
...
def get_thumbnail_cache_stats(maibot_session: Optional[str] = auth_cookie()) -> ThumbnailCacheStatsResponse:
    """获取缩略图缓存统计信息。
...
def cleanup_thumbnail_cache(maibot_session: Optional[str] = auth_cookie()) -> ThumbnailCleanupResponse:
    """清理孤立的缩略图缓存。
...
def clear_all_thumbnail_cache(maibot_session: Optional[str] = auth_cookie()) -> ThumbnailCleanupResponse:
    """清空所有缩略图缓存。
