    ...
    *,
    ...
    for item in items:
        if isinstance(item, AssistantMessageItem):
            ...
            reasoning_text = get_item_text(item).strip()
            if reasoning_text:
    ...
    if include_reasoning_content:
        # DeepSeek thinking 模式要求每条 assistant 历史都回传该字段；即使
        # 当前 assistant 没有显式推理，也必须发送空字符串，否则多轮请求 400。
        payload["reasoning_content"] = reasoning_content  # type: ignore[typeddict-unknown-key]
    return payload
...
class OpenaiClient(AdapterClient[AsyncStream[ChatCompletionChunk], ChatCompletion]):
    ...
    async def _execute_response_request(
        ...
        try:
            ...
            normalized_model_identifier = model_info.model_identifier.strip().lower()
            provider_hostname = (
                urlparse(
                    self.api_provider.base_url
                    if "://" in self.api_provider.base_url
                    else f"//{self.api_provider.base_url}"
                ).hostname
                or ""
            ).lower()
            include_reasoning_content = (
                provider_hostname == "api.deepseek.com"
                or normalized_model_identifier.startswith("deepseek")
            )
